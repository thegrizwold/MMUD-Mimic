...
public sealed partial class MapBuilderService
{
    ...
    private void ChartCell(BuildState st, int cell, long room, long map)
    {
        ...
        if (!opt.NotNpcs && rec.Npc > 0)
        {
            ...
            // 2.3.4: the NPC's greet commands join the room references
            // (AddRoomNPCCommandRefs) — teleporting greets get their own
            // "Teleport: (NPC)" row, the rest share one "Greet:" row
            try
            {
                var refs = db.GetRoomNpcCommandRefs(rec.Npc, map);
                if (refs.Count > 0) sNpcCmds = string.Join("\r\n", refs);
            }
            catch { /* tooltip stays without greets */ }
        }
        ...
        if (!opt.NoTips)
        {
            ...
            tip = AutoAppend(tip, sNpcCmds);
            if (sRemote.Length > 0 || sRoomCmds.Length > 0 || sNpcCmds.Length > 0) tip += "\r\n";
