...
public partial class MainViewModel
{
    ...
    internal bool MonsterPassesPanel(MonsterBrowseRow m)
    {
        ...
        // lair mode: the filter tests the lair-avg HP when the mob has lairs
        // (frmMain :25698); the COLUMN shows the mob's own HP since 2.3.4
        long hpForFilter = MonsterByLair && m.LairAvgHp >= 0 ? m.LairAvgHp : m.Hp;
        if (hpForFilter > MonHpMax) return false;
