...
public sealed class CharacterProfileService
{
        ...
        long nWeaponNumber = 0, bool bForceNoChar = false)
    {
        ...
        // 2.3.4 FIX (:5377): a bForceNoParty profile (spell filtering in
        // monster lair mode) must not inherit the lair party size, or the
        // party>1 branch below skips the character and spells stop filtering
        if (!bForceNoParty && ui.PartyFilterOn && ui.PartySizeText > 1)
