...
public sealed class DamageOutputService
{
        ...
        bool bForceCharacter = false)
    {
        ...
        string sAttackDesc = "", sAttackDetail = "";
        ...
        if (!oneshotDone)
        {
            ...
            if (nParty > 1 || cfg.AttackType == MmeAttackType.Manual)
            {
                ...
                if (nDmgPhysical > 0)
                {
                    ...
                    sAttackDesc = tAttack.SAttackDesc; sAttackDetail = tAttack.SAttackDetail;
                }
                if (nDmgSpell > 0)
                {
            ...
            else
            {
                switch (cfg.AttackType)
                {
                    ...
                    case MmeAttackType.SpellAny:
                    {
                        ...
                        // 2.3.4 FIX: immunity is compared against the spell's required
                        // level (falls back to the cast level when ReqLevel = 0)
                        if (nSpellImmuLvl == 0
                            || tSpellcast.RequiredLevel > nSpellImmuLvl
                            || (tSpellcast.RequiredLevel == 0
                                && tSpellcast.CastLevel > nSpellImmuLvl))
                        {
    ...
    private static DamageOutput Assemble(decimal avg, decimal first, decimal minRound,
        ...
        string desc = "", string detail = "") =>
        new()
        {
            SAttackDesc = desc,
            SAttackDetail = detail,
            NAverageDamage = avg,
