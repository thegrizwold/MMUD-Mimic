...
///  - HP column: ALWAYS the mob's own HP (2.3.4 dropped the lair-average
///    HP + "*" from the column); the HP ≤ filter still uses the lair
///    average (LairAvgHp) as frmMain :25698 does;
...
public partial class MainViewModel
{
        ...
        IEnumerable<Mme.Data.MonsterBrowseRow> rows)
    {
        ...
        foreach (var m in rows)
        {
            ...
            // frmMain :25698 (unchanged in 2.3.4): the HP ≤ filter tests the
            // lair-average HP whenever the mob has lairs
            long lairAvgHp = li.NTotalLairs > 0 ? li.NAvgHp : -1;
            double dmgResolved;
            if (lairAvgMode)
            {
                // 2.3.4 (AddMonster2LV :5950): the HP column no longer shows
                // the combined lair HP in lair mode — it is always the
                // monster's own HP (and so is the HP filter's Tag). Damage
                // keeps the lair average with its "*" marker.
            ...
            yield return m with
            {
                ...
                Hp = hpForFilter, // 2.3.4: the column shows monster HP, never the lair average
                LairAvgHp = lairAvgHp,
            };
        }
    }
