...
public partial class MainViewModel
{
        ...
        : System.ComponentModel.INotifyPropertyChanged
    {
        ...
        private long _qty = 1;
        /// <summary>Row quantity. 2.3.4: the flag's " xN" suffix is the
        /// quantity's source of truth (bare flag = 1), so QTY and a flagged
        /// row's suffix are kept in step both ways.</summary>
        public long Qty
        {
            get => _qty;
            set
            {
                long q = Math.Max(1, value);
                if (_qty == q) return;
                _qty = q;
                Notify(nameof(Qty));
                Notify(nameof(TotalEnc));
                if (_flag.Length > 0)
                {
                    var (b, _) = ParseActionAndQty(_flag);
                    _flag = b + (q > 1 ? $" x{q}" : "");
                    Notify(nameof(Flag));
                }
            }
        ...
        public string Flag
        {
            ...
            set
            {
                var (baseText, qty) = ParseActionAndQty(value ?? "");
                if (baseText.Length == 0) { _flag = ""; Notify(nameof(Flag)); return; }
                // 2.3.4 (LV_SetActionCell): an explicit " xN" is the qty;
                // a bare flag seeds its suffix from the QTY column
                if (qty > 1 && qty != _qty)
                {
                    _qty = qty;
                    Notify(nameof(Qty));
        ...
        /// <summary>LV_SetActionCell 15/16: −/+ quantity (min 1; the flag
        /// suffix drops back to the bare word at 1).</summary>
        public void BumpQty(int delta) => Qty = Math.Max(1, Qty + delta);
    ...
    /// <summary>2.3.4: the Item Manager's +/- quantity buttons (now side by
    /// side) adjust every selected row.</summary>
    public void ImBumpQty(IEnumerable<ImRowVm> rows, int delta)
    {
        foreach (var r in rows) r.BumpQty(delta);
        RefreshImSummary();
    }
    ...
    /// names).</summary>
    private ImRowVm? _imSelectedRow;
    ...
    public void ImSelect(ImRowVm? row)
    {
        ...
        else
        {
            ...
            // lvItemManagerLoc: % desc (stable), then the 2.3.4 shops-first option
            static decimal Pct(string l)
            {
                var m = System.Text.RegularExpressions.Regex.Match(l, @"\(([\d.]+)%\)\s*$");
                return m.Success && decimal.TryParse(m.Groups[1].Value,
                    System.Globalization.NumberStyles.Float,
                    System.Globalization.CultureInfo.InvariantCulture, out var v) ? v : 0;
            }
            ImLocations = OrderRefs(locs.OrderByDescending(Pct).ToList());
        }
