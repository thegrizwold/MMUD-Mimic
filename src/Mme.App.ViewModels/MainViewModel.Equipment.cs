...
public sealed partial class MainViewModel
{
    ...
    public IReadOnlyList<EquipSlotVm> EquipSlots =>
        _equipSlotVms ??= Enumerable
            ...
            .Select(i => new EquipSlotVm(this, i, FilteredSlotList(i)))
            .ToList();
    ...
    private long CurrentBaseStat(int index) => index switch
    {
        0 => (long)CharStr, 1 => (long)CharInt, 2 => (long)CharWil,
        3 => (long)CharAgi, 4 => (long)CharHea, 5 => (long)CharCha, _ => 0,
    };
    ...
    /// <summary>2.3.4 AccumItemStatBonus (frmMain :37676): total the +stat
    /// abilities (46 str, 44 int, 45 wis/wil, 48 agi, 47 hea, 49 cha) that
    /// CalcCharacterStats will re-add for items, indexed like the stat
    /// boxes. Worn items count once; pasted inventory items that apply
    /// their abilities while merely carried (ItemType 10, or armour worn
    /// nowhere) and are usable by the character count ×qty — except +str,
    /// which the encumbrance pass adds once. Only GreaterMUD/Paramud items
    /// add stats; in stock only spells do.</summary>
    private long[] PastedItemStatBonus(Mme.Data.GameTextPasteService.PasteResult parsed)
    {
        var bonus = new long[6];
        if (_db is null || !GreaterMud) return bonus;
        _optimizer ??= new Mme.Data.EquipOptimizerService(_db);

        // AccumItemStatBonus: returns true when anything was added
        bool Accum(long n, bool carried, long qty)
        {
            var row = _optimizer.GetRow(n);
            if (row is null) return false;
            if (qty < 1) qty = 1;
            bool added = false;
            for (int z = 0; z <= 19; z++)
            {
                int idx = row.Abil[z] switch
                { 46 => 0, 44 => 1, 45 => 2, 48 => 3, 47 => 4, 49 => 5, _ => -1 };
                if (idx < 0 || row.AbilVal[z] == 0) continue;
                long multi = carried && idx != 0 ? qty : 1;
                bonus[idx] += row.AbilVal[z] * multi;
                added = true;
            }
            return added;
        }
        ...
        for (int x = 0; x <= 19; x++)
            if (_equipSelected[x] > 0) Accum(_equipSelected[x], false, 1);

        if (parsed.PastedInventory && parsed.CarriedNames.Count > 0)
        {
            // ItemIsUsableByChar(n, True): level/alignment gates always apply,
            // the class part only when a class is selected (class 0 skips it
            // inside IsUsable)
            HashSet<long> usable = new Mme.Data.ItemUsabilityService(_db, GreaterMud)
                .GetUsableItemNumbers((long)CharLevel, CharClassNumber,
                    CharAlignment, isEquipped: n => _equipSelected.Contains(n));
            // frmMain :37517: GetItemsByExactNameArr per pasted name - the
            // FIRST same-named record that is carry-active, usable and adds a
            // stat wins (Exit For); duplicate-named items no longer drop the
            // bonus because the name resolved to a stat-less record
            foreach (var (name, q) in parsed.CarriedNames)
            {
                foreach (long n in _db.FindItemNumbersByExactName(name, OnlyInGame))
                {
                    var b = _db.GetItemBasics(n);
                    if (b is null) continue;
                    bool carryActive = b.Value.ItemType == 10
                        || (b.Value.ItemType == 0 && b.Value.Worn == 0);
                    if (!carryActive) continue;
                    if (!usable.Contains(n)) continue;
                    if (Accum(n, true, q)) break; // only one record can be the one carried
                }
            }
        }
        return bonus;
    ...
    public string ApplyGameTextPaste(string text)
    {
        ...
        long[] itemBonus = new long[6];
        string[] statLabels = ["Strength", "Intellect", "Willpower",
        ...
        try
        {
            ...
            // 2.3.4 FIX (PasteCharacter :37490 / ApplyPastedStat): the game
            // reports each stat WITH its item bonuses applied, but the
            // character sheet holds the BASE stat — take the item portion
            // (worn + carry-active pasted items, GMUD only) back off so
            // base + item bonus = what the game printed. A '*' (spell-
            // modified) stat still goes to the confirm dialog, with the
            // item portion already removed from the suggested value.
            itemBonus = PastedItemStatBonus(parsed);
            PendingModifiedStats.Clear();
            for (int i = 0; i < 6; i++)
            {
                if (parsed.Stats[i] <= 0) continue;
                long baseStat = Math.Max(1, parsed.Stats[i] - itemBonus[i]);
                bool buffed = parsed.ModifiedStats.Any(m => statLabels[i]
                    .StartsWith(m, StringComparison.OrdinalIgnoreCase));
                if (!buffed) SetBaseStat(i, baseStat);
                else if (CurrentBaseStat(i) != baseStat) // ApplyPastedStat: no prompt when already right
                    PendingModifiedStats.Add(new ModifiedStatInfo(i,
                        statLabels[i], parsed.Stats[i], baseStat));
            }
        ...
        if (PendingModifiedStats.Count > 0)
            sb.Append("\n\nStats marked '*' in game are spell-modified — " +
                "the spell portion can't be read from the paste, so confirm " +
                "these base values (item bonuses already taken off): "
                + string.Join(", ", PendingModifiedStats.Select(m => m.Label)) + ".");
        if (itemBonus.Any(b => b != 0))
            sb.Append("\n\nItem stat bonuses taken back out of the pasted stats: "
                + string.Join(", ", statLabels.Select((l, i) => (l, b: itemBonus[i]))
                    .Where(t => t.b != 0)
                    .Select(t => $"{t.l} {(t.b > 0 ? "+" : "")}{t.b}")) + ".");
    ...
    /// <summary>VB6 2.3.4: mnuFindBestArmour(6) "VileWard" is only visible
    /// when bGreaterMUD — criteria built on abilities ≥ 1000 are hidden on
    /// a stock database.</summary>
    public IReadOnlyList<Mme.Data.EquipOptimizerService.Criterion>
        FindBestCriteria => GreaterMud
            ? Mme.Data.EquipOptimizerService.Criteria
            : Mme.Data.EquipOptimizerService.Criteria
                .Where(c => !c.GreaterMudOnly).ToList();
    ...
    /// <summary>Re-publishes the criteria list after a GreaterMUD toggle
    /// and drops a now-hidden selection back to the first entry.</summary>
    internal void NotifyFindBestCriteria()
    {
        if (_selectedCriterion is not null && !GreaterMud
            && _selectedCriterion.GreaterMudOnly)
            _selectedCriterion = null;
        OnChanged(nameof(FindBestCriteria));
        OnChanged(nameof(SelectedCriterion));
    }
    ...
    public string RunFindBest(bool nextBest)
    {
        ...
        var run = _optimizer.FindBestEx(SelectedCriterion, lists,
            _equipSelected, _equipHold, nextBest, NoLimitedItems,
            _findBestState, greaterMud: GreaterMud);
        var picks = run.Picks;
        ...
        RefreshEquipSlotVms();
        RecalcEquipment();
        string msg = !run.Found ? "Nothing found." // VB6 MsgBox text
    ...
    private void NotifyEquipPanel()
    {
        ...
        OnChanged(nameof(EqAbilityTotals)); // Beta 31 caster readout
