...
public sealed record WeaponBrowseRow(long Number, string Name, string Type,
    ...
    long WeaponTypeNum = 0, long Extra = 0)
{
    ...
    long Limit, string AcPerEnc, long Magical = 0)
{
    ...
    /// <summary>2.3.4 (SetArmourACDRSortTags): numeric sort tags for the
    /// AC/DR column, built from the displayed "ac/dr" text ×10 exactly like
    /// the VB6 — AC-major <c>(ac*100000)+dr</c> and DR-major
    /// <c>(dr*100000)+ac</c>. The grid alternates between them on repeated
    /// header clicks.</summary>
    public long AcSortKey => AcDrKeys().Ac * 100000 + AcDrKeys().Dr;
    public long DrSortKey => AcDrKeys().Dr * 100000 + AcDrKeys().Ac;

    private (long Ac, long Dr) AcDrKeys()
    {
        var parts = (AcDr ?? "").Split('/');
        if (parts.Length != 2) return (0, 0);
        double.TryParse(parts[0], System.Globalization.NumberStyles.Float,
            System.Globalization.CultureInfo.InvariantCulture, out double ac);
        double.TryParse(parts[1], System.Globalization.NumberStyles.Float,
            System.Globalization.CultureInfo.InvariantCulture, out double dr);
        return ((long)Math.Round(ac * 10, MidpointRounding.ToEven),
                (long)Math.Round(dr * 10, MidpointRounding.ToEven));
    }
}
...
{
    /// <summary>AddMonster2LV "Spell Atk." (:6306): the attack-type letters of
    /// the monster's spell attacks (C/F/S/L/N/W/P) "+" the extra-hit /
    /// between-round spell types, each part letter-sorted. String column,
    /// sorted as text.</summary>
    public string SpellAtk { get; init; } = "";
    ...
    /// <summary>frmMain :25698 — in lair mode the HP ≤ filter still tests
    /// tLastAvgLairInfo.nAvgHP (2.3.4 only removed the lair HP from the
    /// COLUMN). -1 = no lair data → filter on the mob's own HP.</summary>
    public long LairAvgHp { get; init; } = -1;
    ...
    /// <summary>2.3.4 (AddMonster2LV :6297): BS Defense, blank when 0.</summary>
    public string BsDefenseText => BsDefense > 0 ? BsDefense.ToString() : "";
...
public sealed partial class MmeDatabase
{
        ...
        Mme.Core.Model.CharacterProfile profile, IGameEngineRules rules)
    {
        ...
        while (r.Read())
        {
            ...
            try
            {
                ...
                // 2.3.4 (AddWeapon2LV :4964): "Extra" reflects the hit chance,
                // so xSwings + Extra = the average round. VB6 Round = banker's.
                if (dmg.AvgExtraSwing > 0 && dmg.Swings > 0 && dmg.HitChance > 0)
                    extra = Mme.Core.Text.VbRuntime.CLng(dmg.AvgExtraSwing * dmg.Swings
                        * (dmg.HitChance / 100.0));
            ...
            list.Add(new WeaponBrowseRow(number, S(r[1]),
                ...
                magical, L(r[2]), extra) { Abils = pairs });
        }
        return list;
    }
    ...
    public List<MonsterBrowseRow> GetMonsterBrowseRows(IGameEngineRules rules)
    {
        ...
        var spellAtk = GetMonsterSpellAttackTypes();
        ...
        while (r.Read())
        {
                ...
                D(r[9]), mag, L(r[10]) == 1 ? "X" : "")
            {
                ...
                MonAbils = monAbils,
                SpellAtk = spellAtk.TryGetValue(L(r[0]), out var sa) ? sa : "",
            });
        }
        return list;
    }

    /// <summary>GetMonsterAttackSummary(nNum, bGetSpellAttackTypes:=True)
    /// for every monster + the AddMonster2LV :6306 assembly:
    /// sSpellAttackTypes = letters of each AttType-2 attack's spell
    /// (SpellAttackTypeEnum(GetSpellAttackType, bShort)); sSpellExtraTypes =
    /// letters of MidSpells (running-nPercent quirk, &gt; 0 only) and
    /// AttHitSpells that SpellDoesDamage(…, True); result = types &amp;
    /// "+" &amp; extras when extras exist, then SortLettersWithSeparator.</summary>
    public Dictionary<long, string> GetMonsterSpellAttackTypes()
    {
        // spells: Number → AttType, and a resolver for SpellDoesDamage
        var attType = new Dictionary<long, int>();
        var cache = new Dictionary<long, SpellRecord?>();
        using (var sc = _con.CreateCommand())
        {
            sc.CommandText = "SELECT \"Number\",\"AttType\" FROM \"Spells\"";
            using var sr = sc.ExecuteReader();
            while (sr.Read()) attType[L(sr[0])] = checked((int)L(sr[1]));
        }
        SpellRecord? Resolve(long n)
        {
            if (!cache.TryGetValue(n, out var rec)) { rec = GetSpellRecord(n); cache[n] = rec; }
            return rec;
        }
        string Letter(long spell) => spell == 0 || !attType.TryGetValue(spell, out int t)
            ? EnumNames.SpellAttackTypeEnum(0, true)       // GetSpellAttackType miss → 0 → "C"
            : EnumNames.SpellAttackTypeEnum(t, true);

        var result = new Dictionary<long, string>();
        var sb = new System.Text.StringBuilder("SELECT \"Number\"");
        for (int x = 0; x <= 4; x++)
            sb.Append($",\"AttType-{x}\",\"Att%-{x}\",\"AttAcc-{x}\",\"AttHitSpell-{x}\"");
        for (int x = 0; x <= 4; x++)
        ...
        while (r.Read())
        {
            ...
            for (int x = 0; x <= 4; x++)
            {
                long spell = L(r[21 + x * 2]);
                if (spell == 0) continue;
                nPercent = L(r[22 + x * 2]) - nPercent;
                if (nPercent > 0 && SpellDamageMath.SpellDoesDamage(Resolve, spell, true))
                    extras += Letter(spell);
            ...
            for (int x = 0; x <= 4; x++)
            {
                ...
                if (type == 2) types += Letter(L(r[c + 2]));
                long hit = L(r[c + 3]);
                if (hit > 0 && SpellDamageMath.SpellDoesDamage(Resolve, hit, true))
                    extras += Letter(hit);
            }
            if (extras.Length > 0) types += "+" + extras;
            if (types.Length > 0) types = TextUtils.SortLettersWithSeparator(types, "+");
            if (types.Length > 0) result[L(r[0])] = types;
        }
        return result;
    }
    ...
    /// <summary>PullSpellDetail tail: Target / Difficulty / Attack Type +
    /// the ability list. 2.3.4: "Difficulty:" is shown when
    /// <c>Diff &lt; 200 And (Diff &lt;&gt; 0 Or bSpellCanBeLearned)</c>, where a
    /// spell can be learned when <c>Learnable = 1</c> or it is a Kai power
    /// (<c>Magery = 5</c>) with autolearn on and <c>ReqLevel &gt; 0</c> — so a
    /// learnable spell with difficulty 0 no longer omits the line.</summary>
    public string GetSpellAbilityText(long number, IGameEngineRules rules,
        bool disableKaiAutolearn = false)
    {
        using var cmd = _con.CreateCommand();
        var sql = new System.Text.StringBuilder(
            "SELECT \"Targets\",\"Diff\",\"AttType\",\"Learnable\",\"Magery\",\"ReqLevel\"");
        for (int i = 0; i <= 9; i++) sql.Append($",\"Abil-{i}\",\"AbilVal-{i}\"");
        ...
        long diff = L(r[1]);
        bool canBeLearned = L(r[3]) == 1
            || (L(r[4]) == 5 && !disableKaiAutolearn && L(r[5]) > 0);
        var sb = new System.Text.StringBuilder();
        sb.Append("Target: ").Append(EnumNames.GetSpellTargetsEnum(checked((int)L(r[0]))));
        // 2.3.4: show difficulty even when it is 0, but only for spells
        // that can actually be learned (:4433)
        if (diff < 200 && (diff != 0 || canBeLearned))
