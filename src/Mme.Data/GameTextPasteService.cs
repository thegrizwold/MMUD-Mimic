    ...
    SpellUsabilityService? spellUsability = null, long classNumber = 0)
{
    public sealed class PasteResult
    {
        ...
        /// <summary>The pasted inventory names as written ("Name (N)" already
        /// split into name + qty), in paste order — the 2.3.4 pasted-stat
        /// bonus walks every same-named item record for each (frmMain :37517
        /// GetItemsByExactNameArr), not just the resolved number.</summary>
        public List<(string Name, long Qty)> CarriedNames { get; } = [];
    ...
    private static bool TestPasteChar(char c) =>
        Mme.Core.Formulas.MudParse.TestPasteChar(c.ToString()); // full VB6 set incl. ' " . ` digits
    ...
    public PasteResult Parse(string search)
    {
        ...
        // 2.3.4 PastedStatIsBuffed (frmMain :37691): the game right-aligns
        // the value and floats the "*" ahead of it, so its column moves with
        // the digit count — skip spaces rather than matching a fixed number
        // of them (the pre-2.3.4 fixed spellings missed 2-digit stats).
        string[] statLabels =
            ["Strength:", "Intellect:", "Willpower:", "Agility:", "Health:", "Charm:"];
        for (int i = 0; i < 6; i++)
        {
            long v = ExtractValueFromString(search, statLabels[i]);
            if (v <= 0) continue;
            res.Stats[i] = v;
            if (PastedStatIsBuffed(search, statLabels[i]))
                res.ModifiedStats.Add(statLabels[i].TrimEnd(':'));
        }
        ...
        if (res.PastedInventory)
        {
            ...
            {
                if (!isKey) res.CarriedNames.Add((name, qty));
    ...
    /// <summary>frmMain :37691 PastedStatIsBuffed — true when the stat
    /// following <paramref name="label"/> is asterisked (spell-modified).
    /// Finds the label the same way ExtractValueFromString does so the two
    /// always read the same stat.</summary>
    internal static bool PastedStatIsBuffed(string search, string label)
    {
        int x = search.IndexOf(label, StringComparison.OrdinalIgnoreCase);
        if (x < 0) return false;
        x += label.Length;
        while (x < search.Length && search[x] == ' ') x++;
        return x < search.Length && search[x] == '*';
    }
